...
| `in-style`               | `left-align`, **`right-align`**, `no-align`           | How to align the `in` keyword with respect to `let` (`left-align` produces <code>in &nbsp;...</code>, `right-align` produces <code>&nbsp;in ...</code>, `no-align` produces <code>in ...</code>)
