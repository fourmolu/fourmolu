...
p_stringLit src = case parseStringLiteral $ T.pack $ unpackFS src of
  ...
  Just StringLiteralData {content = kind, ..} -> sitcc do
    txt startMarker
    case kind of
      RegularStringLiteral segs -> do
        let singleLine =
              txt $ T.concat segs
            multiLine =
              sep breakpoint f (attachRelativePos segs)
              where
      ...
      MultilineStringLiteral segs ->
        sep breakpoint' txt segs
    txt endMarker
...
parseStringLiteral = \s -> do
  kind <-
    (MultilineStringLiteral <$> stripStartEndMarker "\"\"\"" s)
      <|> (RegularStringLiteral <$> stripStartEndMarker "\"" s)
  pure case kind of
    RegularStringLiteral dat ->
      RegularStringLiteral . splitGaps <$> dat
    MultilineStringLiteral dat ->
      MultilineStringLiteral . transform <$> dat
      where
        -- See the the MultilineStrings GHC proposal and 'lexMultilineString'
        -- from "GHC.Parser.String" for reference.
        --
  ...
  where
    ...
    stripStartEndMarker :: Text -> Text -> Maybe (StringLiteralData Text)
    stripStartEndMarker marker s = do
      ...
      pure StringLiteralData {content = infix_, ..}
...
-- | The start/end marker of a literal, plus some content between them.
data StringLiteralData a = StringLiteralData
...
data StringLiteralKind a = RegularStringLiteral a | MultilineStringLiteral a
