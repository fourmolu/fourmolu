    ...
    switchLayout conDeclSpn $ do
      ...
      inci $ do
        ...
        conTy <- case con_args of
          ...
          RecCon r@(L l rs) -> pure
            . L (combineLocs r con_res_ty)
            $ HsFunTy NoExtField (L l $ HsRecTy NoExtField rs)
                con_res_ty
          InfixCon _ _ -> notImplemented "InfixCon"
        let qualTy = case con_mb_cxt of
              Nothing -> conTy
              Just qs -> L (combineLocs qs conTy)
                $ HsQualTy NoExtField qs conTy
        let quantifiedTy = if unLoc con_forall
              then L (combineLocs con_forall qualTy)
