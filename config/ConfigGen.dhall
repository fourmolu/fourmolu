...
let list = \(fieldType : data.EnumType) -> Prelude.Text.concatMap {index: Natural, value: data.Enum}
    (\(enum:  {index: Natural, value: data.Enum}) -> "\\\"${data.showEnumPretty enum.value}\\\"" ++
      (if Prelude.Natural.greaterThan (Prelude.List.length data.Enum fieldType.constructors) 2
        && Prelude.Natural.lessThan (enum.index + 1) (Prelude.List.length data.Enum fieldType.constructors)
          then "," else "") ++
      (if Prelude.Natural.lessThan (enum.index + 1) (Prelude.List.length data.Enum fieldType.constructors)
          then " " else "") ++
      (if Prelude.Natural.equal (enum.index + 2) (Prelude.List.length data.Enum fieldType.constructors)
          then "or " else "")
)
...
-- let w = Prelude.List.in

-- let w = if 1 == 1 then 2 else 2
...
data PrinterOpts f =
  PrinterOpts
${Prelude.Text.concatMap {index: Natural, value: data.Option}
    -- (\(option: data.Option) -> "      ${option.fieldName} :: f ${option.type_}")
    (\(option: {index: Natural, value: data.Option}) ->
let lead = if Prelude.Natural.isZero option.index then "{" else "," in
''
    ${lead} -- | ${option.value.description}
      ${option.value.fieldName} :: f ${data.showType option.value.type_}
''
)
...
let lead = if Prelude.Natural.isZero option.index then "{" else "," in
''
    ${lead} ${option.value.fieldName} = pure ${data.showValue option.value.default_}
''
)
    (Prelude.List.indexed data.Option data.options)
}    }
...
let choices = merge
  ...
  option.value.type_
let default = "${option.value.description} ${choices}(default: ${data.showValuePretty option.value.default_})"
in
''
    <*> f
      "${option.value.name}"
      "${merge {Enum = \(x: data.EnumType) -> default, Bool = default, Natural = default, Text = default, ADT = \(x: data.ADT) -> x.cli} option.value.type_}"
...
${Prelude.Text.concatMap {index: Natural, value: Text}
    (\(enum: {index: Natural, value: Text}) ->
let lead = if Prelude.Natural.isZero enum.index then "=" else "|" in
''
  ${lead} ${enum.value}
''
)
  --TODO why highlight?
    (Prelude.List.indexed Text (

merge {
  ,Enum = \(x: data.EnumType) -> Prelude.List.map data.Enum Text data.showEnum x.constructors
...
''
  parsePrinterOptType s =
    case s of
${Prelude.Text.concatMap  data.Enum
    (\(enum:  data.Enum) ->
''
